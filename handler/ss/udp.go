...
func udpLocal(laddr, server, target string, shadow func(net.PacketConn) net.PacketConn) {
	...
	if err != nil {
		log.Println(fmt.Sprintf("UDP server address error: %v", err))
		return
	}
	...
	if tgt == nil {
		...
		log.Println(fmt.Sprintf("UDP target address error: %v", err))
		return
	}

	c, err := net.ListenPacket("udp", laddr)
	if err != nil {
		log.Println(fmt.Sprintf("UDP local listen error: %v", err))
		return
	}
	defer c.Close()
	...
	log.Println(fmt.Sprintf("UDP tunnel %s <-> %s <-> %s", laddr, server, target))
	for {
		n, raddr, err := c.ReadFrom(buf[len(tgt):])
		if err != nil {
			log.Println(fmt.Sprintf("UDP local read error: %v", err))
			continue
		}
		...
		if err != nil {
			log.Println(fmt.Sprintf("UDP local write error: %v", err))
			continue
		}
	}
}
...
func udpSocksLocal(laddr, server string, shadow func(net.PacketConn) net.PacketConn) {
	...
	for {
		...
		if pc == nil {
			...
			log.Println(fmt.Sprintf("UDP socks tunnel %s <-> %s <-> %s", laddr, server, socks.Addr(buf[3:])))
			pc = shadow(pc)
...
func UdpRemoteConn(addr string, shadow func(net.PacketConn) net.PacketConn) {
	udpRemote(addr, shadow)
}
...
func udpRemote(addr string, shadow func(net.PacketConn) net.PacketConn) {
	...
	if err != nil {
		log.Println(fmt.Sprintf("UDP remote listen error: %v", err))
		return
	}
	defer c.Close()
	c = shadow(c)
	...
	log.Println(fmt.Sprintf("listening UDP on %s", addr))
	for {
		n, raddr, err := c.ReadFrom(buf)
		if err != nil {
			log.Println(fmt.Sprintf("UDP remote read error: %v", err))
			continue
		}

		tgtAddr := socks.SplitAddr(buf[:n])
		if tgtAddr == nil {
			log.Println(fmt.Sprintf("failed to split target address from packet: %q", buf[:n]))
			continue
		}

		tgtUDPAddr, err := net.ResolveUDPAddr("udp", tgtAddr.String())
		if err != nil {
			log.Println(fmt.Sprintf("failed to resolve target UDP address: %v", err))
			continue
		}
		...
		if err != nil {
			log.Println(fmt.Sprintf("UDP remote write error: %v", err))
			continue
		}
	}
}
